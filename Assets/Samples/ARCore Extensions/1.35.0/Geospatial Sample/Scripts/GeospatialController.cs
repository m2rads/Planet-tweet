...
namespace Google.XR.ARCoreExtensions.Samples.Geospatial
{
    ...
    public class GeospatialController : MonoBehaviour
    {
        ...
        private List<GeospatialAnchorHistory> _histories = new List<GeospatialAnchorHistory>();
        ...
        public void GetTweets() {
            twitterAPIRequest = new TwitterAPIRequest();
...
#endif
            string coordinates = pose.Latitude + "," + pose.Longitude;

            string response = "";
            yield return StartCoroutine(twitterAPIRequest.GetData(coordinates));
            response = twitterAPIRequest.GetData(coordinates).Current as string;
            TweetResponseObject model = JsonConvert.DeserializeObject<TweetResponseObject>(response);
            Debug.LogError(response);

            // fill the array with histories
            foreach (var item in model.statuses)
            {
                var lat = item.place.bounding_box.coordinates[0][0][0];
                var lon = item.place.bounding_box.coordinates[0][0][1];

                _histories.Add(new GeospatialAnchorHistory(lat, lon, pose.Altitude, eunRotation, item));
            }
        }
        ...
        public void OnSetAnchorClicked()
        {
            ...
            // pose.Latitdue pose.Longitude pose.Altitude
//             var pose = EarthManager.CameraGeospatialPose;
//             Quaternion eunRotation = pose.EunRotation;
// #if UNITY_IOS
//             // Update the quaternion from landscape orientation to portrait orientation.
...
// #endif
            ...
            var anchor = PlaceGeospatialAnchor(_histories[0], _usingTerrainAnchor);
            if (anchor != null)
            {
            ...
            for (int i = 1; i < 3; i++) {
                anchor = PlaceGeospatialAnchor(_histories[i], _usingTerrainAnchor);
                if (anchor != null)
                {
        ...
        public void Update()
        {
                ...
                && !EventSystem.current.IsPointerOverGameObject(Input.GetTouch(0).fingerId))
            {
                ...
                Debug.Log("Can't place anchor by tap");
            }
            ...
            GeospatialAnchorHistory history, bool terrain = false)
        {
            ...
            if (anchor != null)
            {
                ...
                anchorGO.transform.Find("Tweet").GetComponent<Text>().text = history._Status.text;
                anchorGO.transform.Find("Handler").GetComponent<Text>().text = history._Status.user.name;
